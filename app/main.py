...
async def evaluate(request: EvaluationRequest):
    ...
    Reports:
    - ROUGE-1/2/L scores
    - Recall@1, Recall@3, Recall@5, Recall@10 (IR metrics)
    - MRR (Mean Reciprocal Rank)
    - nDCG@5, nDCG@10
    - LLM-as-judge: faithfulness, relevance, coherence (when enable_llm_judge=True)
    - Hallucination score, theme detection accuracy, latency
    """
    experiment_runner.enable_llm_judge = request.enable_llm_judge
    ...
    return EvaluationResponse(
        query=request.query,
        results=results,
        best_mode=experiment["best_mode"],
        improvement_over_tfidf_pct=experiment.get("improvement_over_tfidf_pct", 0.0),
        target_met=experiment.get("target_met", False),
    )
...
async def get_costs():
    """
    Token usage and cost analytics per query.
    Shows: cost per query, cost per 1K queries, by model breakdown.
    Useful for capacity planning and cost optimization decisions.
    """
